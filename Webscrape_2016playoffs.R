...
start <- as.Date("16-04-16", format="%d-%m-%y")
end <- as.Date("19-06-16", format="%d-%m-%y")
...
## assumes all the scrapped information is in the subfolder "Outcomes"

res <- list()
game.id <- 1
for(fname in list.files("Playoffs_scripts")){
  tmp <- read.csv(paste0("Playoffs_scripts/", fname))
  res <- rbind(res, cbind(game.id=game.id, tmp))
  game.id <- game.id + 1
}

write.csv(res, "allgamess.csv", row.names = FALSE)
